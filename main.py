...
# loading levels
for row in range(scripts.constants.ROWS):
    r = [-1] * scripts.constants.COLS
    world_data.append(r)
with open("levels/level1_ground.csv", newline="") as csvfile:
    reader = csv.reader(csvfile, delimiter= ",")
    for x, row in enumerate(reader):
        for y, tile in enumerate(row):
            world_data[x][y]= int(tile)
player, enemy_list, boss= world.process_date(world_data, "castle")

for row in range(scripts.constants.ROWS):
    r = [-1] * scripts.constants.COLS
    world_mobs_data.append(r)
with open("levels/level1_mobs.csv", newline="") as csvfile:
    reader = csv.reader(csvfile, delimiter= ",")
    for x, row in enumerate(reader):
        for y, tile in enumerate(row):
            world_mobs_data[x][y]= int(tile)
player, enemy_list, boss_list = mobs_world.process_date(world_mobs_data, "castle")
